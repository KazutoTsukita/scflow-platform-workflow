...
the whole SDL stage (lookup, transfer, and integrity checking) has a six-hour
limit. Set `UNISCFLOW_SDL_LOOKUP_TIMEOUT_SECONDS` and
`UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS` to positive finite seconds to adjust these
limits, for example for unusually large source files. A timeout stops the SDL
workers and their child processes, retains downloaded files and completed
results, and records a failure reason. Final selected-run coverage and integrity
checks still determine whether the download stage can succeed.
