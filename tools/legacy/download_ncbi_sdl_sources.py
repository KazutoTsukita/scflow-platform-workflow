...
def main() -> int:
    ...
    parser.add_argument("--stage-timeout-seconds", type=float,
                        default=os.environ.get("UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS", str(48 * 3600)))
    ...
    timed_out = [row for row in failed if row.get("reason") == "sdl_stage_timeout"]
    if timed_out:
        print(f"[WARNING] SDL fallback reached its {args.stage_timeout_seconds / 3600:g} h limit with "
              f"{len(timed_out)} run(s) unfinished; partial files are kept and resume on the next run. "
              "Rerun the project, or raise the limit with --sdl-stage-timeout-hours.", file=sys.stderr, flush=True)
