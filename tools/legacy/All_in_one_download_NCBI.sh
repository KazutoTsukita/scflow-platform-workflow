...
show_help() {
    ...
    echo "  sdl_stage_timeout_seconds: Wall-clock limit for the NCBI SDL source-file fallback of one project; partial files resume on the next run (default: UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS, otherwise 172800 = 48 h)"
...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        fastq_integrity_retries=*) fastq_integrity_retries=${key#*=}
                    shift # Remove the argument
                    ;;
        sdl_stage_timeout_seconds=*) sdl_stage_timeout_seconds=${key#*=}
                    shift # Remove the argument
                    ;;
...
if [ -n "${sdl_stage_timeout_seconds}" ] && ! [[ "${sdl_stage_timeout_seconds}" =~ ^[1-9][0-9]*$ ]]; then
    echo "Error: sdl_stage_timeout_seconds must be a positive integer"
    exit 1
fi
...
# FASTQ integrity validation reads every file end to end; validate files concurrently with the
# same worker count as fasterq-dump/pigz (a non-numeric value falls back to the serial path).
coverage_parallel="${parallel}"
case "${coverage_parallel}" in ''|*[!0-9]*) coverage_parallel=1 ;; esac
...
if [ "${download_status}" -ne 0 ]; then
    ...
    --parallel "${coverage_parallel}" \
        ...
        if [ "${#sdl_missing_runs[@]}" -eq 0 ]; then
            ...
            if [ -n "${sdl_stage_timeout_seconds}" ]; then
                sdl_args+=(--stage-timeout-seconds "${sdl_stage_timeout_seconds}")
            fi
