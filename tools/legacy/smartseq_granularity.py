...
# A plate named in the GSM identity ("batch: plate1", "Plate 3", "384-well") together with a
# same-GSM statement that single cells were prepared/sorted is plate single-cell context even
# when no single clause joins "single cells" and "plate" (GSE318611: 384 runs per plate GSM,
# "Single cells were prepared using the method described in Li et al. 2017").
PLATE_NAMED_IDENTITY_RE = re.compile(
    r"\bplates?[-_ ]?(?:\d{1,3}|[A-Z])\b|\b(?:96|384)[- ]?well\b",
    re.I,
)
SINGLE_CELL_PREPARATION_RE = re.compile(
    r"\b(?:single|individual)\s+(?:cells?|nuclei|neurons?)\s+were\s+"
    r"(?:prepared|isolated|collected|picked|sorted|dissociated|obtained|captured|hand[- ]?picked)\b",
    re.I,
)
...
def classify_sample(
    ...
    plate_named_identity_single_cell_prep = bool(
        PLATE_NAMED_IDENTITY_RE.search(identity_text)
        and applied_positive_signal(sample_text, SINGLE_CELL_PREPARATION_RE)
    )
    plate_single_cell = bool(
        ...
        or plate_named_identity_single_cell_prep
    )
    ...
    if plate_named_identity_single_cell_prep:
        evidence.append(
            "GSM identity names a plate and the GSM protocol states single cells were prepared"
        )
    return {
        "sample": sample,
