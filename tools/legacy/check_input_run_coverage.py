        ...
        for path in sorted(set(paths)):
            ...
            if (
                ...
                relocated = (
                    None
                    if strict_content
                    else relocated_fastq_cache_entry(
                        project_dir, path, stat, cache, rearrangements
                    )
                )
                if relocated is not None:
                    valid = True
                    reason = relocated.get("reason") or "cached_unknown"
                    if stats is not None:
                        stats["relocated_cache_hits"] += 1
                else:
                    if strict_content:
                        valid, reason = validate_gzip_fastq(
                            path,
                            strict_content=True,
                        )
                    else:
                        valid, reason = validate_gzip_fastq(path)
                    if stats is not None:
