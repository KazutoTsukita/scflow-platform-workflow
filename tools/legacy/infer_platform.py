...
METADATA_RULES = (
    ...
    MetadataRule(
        ...
        "high",
        "bd_rhapsody_name",
        re.compile(r"\bbd\s+rhapsody\b|\brhapsody\s+wta\b|\babseq\b|\bbd\s+platform\b", re.I),
    ),
    ...
    MetadataRule("ramda_seq", "high", "ramda_seq_name", re.compile(r"\bramda[-_\s]?seq\b|\bramdaseq\b", re.I)),
...
def mixed_spatial_protocol_gex_context(
    ...
    return {
        "eligible_gex": bool(
            ...
            and mixed_processing and not conflicts
        ),
        ...
        "conflicting_sample_evidence": conflicts,
    }
...
def chemistry_call(args: argparse.Namespace, metadata: Call | None = None) -> Call | None:
    ...
    return Call(
        ...
        actionable=True,
        subtype=subtype,
        extra={"cellranger_chemistry": report.get("cellranger_chemistry", {})},
    )
...
def strict_bdrhapsody_targeted_panel_scope(
    ...
    for gsm, audit in sorted(sample_audits.items()):
        ...
        if not product or not workflow or not target_count:
            return None
...
def strong_sample_scope_routes(metadata: Call) -> dict[str, object]:
    ...
    for sample in selected:
        ...
        if (
            ...
            if (
                targeted.get("bdrhapsody_targeted_product_evidence")
                and targeted.get("bdrhapsody_targeted_workflow_evidence")
                and targeted.get("bdrhapsody_target_count_evidence")
            ):
            ...
            for protocol in ("smartseq2", "smartseq3"):
                ...
                suppressed_protocol_candidates[protocol] = {
                    "reason": (
                        ...
                        "validated sample-local method or complete bulk consensus "
                        "establishes the competing endpoint"
                    ),
