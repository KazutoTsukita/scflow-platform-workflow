...
def inspect_bam_tags(path: Path, max_records: int | None) -> BamTagEvidence:
    ...
    raw_lengths: set[tuple[int, int]] = set()
    header: list[str] = []
    legacy_provenance = ""
    ...
    try:
        for line in process.stdout:
            ...
            if raw_valid:
                ...
                raw_lengths.add((len(values["CR"]), len(values["UR"])))
            if corrected_valid:
    ...
    if raw_complete_records and len(raw_lengths) == 1:
        # Uniform CR/UR lengths let the STARsolo command state the barcode geometry
        # explicitly instead of relying on STAR's 16+10 defaults (v3 UMIs are 12 nt).
        barcode_length, umi_length = next(iter(raw_lengths))
