...
def is_feature_capture_chemistry_definition(name: str, definition: dict) -> bool:
    """True for Cell Ranger definitions whose barcode whitelist is a translation table.

    The "-CS1" family (SC3Pv3-CS1, SC3Pv4-CS1, ...) describes Feature Barcode libraries
    captured by Gel Bead Capture Sequence 1: their R1 carries the *translated* partner of the
    gel-bead barcode and their R2 carries a feature oligo, not cDNA.  Cell Ranger uses these
    definitions only for feature libraries; mapping such reads as GEX yields an empty or,
    when pooled with GEX lanes (GSE319708 GSM9524173, GSE264124), a barcode-truncated matrix.
    """
    for barcode in definition.get("barcode", []) or []:
        whitelist = (barcode or {}).get("whitelist") or {}
        if isinstance(whitelist, dict) and whitelist.get("translation"):
            return True
    return False
...
def is_standard_10x_gex_definition(name: str, definition: dict) -> bool:
    if is_flex_chemistry_definition(name, definition):
        return False
    if is_feature_capture_chemistry_definition(name, definition):
        return False
    if not (
...
def infer_from_cellranger_chemistry(
    ...
    for name, chem in chemistry_data.items():
        ...
        if not isinstance(chem, dict):
            ...
            continue
        # Feature-capture (translated-whitelist) definitions are scored only as evidence that
        # the reads are a feature library; they never enter the GEX candidate pool unless the
        # caller named them explicitly.
        if is_feature_capture_chemistry_definition(str(name), chem) and not (
            allowed_chemistries and name in allowed_chemistries
        ):
            feature_candidate = evaluate_chemistry(
                name,
                chem,
                stats=stats,
            ...
            if feature_candidate:
                feature_capture_candidates.append(feature_candidate)
            continue
        whitelist_names = {
    ...
    feature_capture_candidates = sorted(
        feature_capture_candidates,
        key=lambda item: (item["score"], item["min_match_rate"]),
        reverse=True,
    )
    best_gex_score = max(
        (float(candidate.get("score") or 0.0) for candidate in candidates), default=0.0
    )
    if (
        feature_capture_candidates
        and float(feature_capture_candidates[0].get("score") or 0.0) >= min_barcode_match_rate
        and best_gex_score < min_barcode_match_rate
    ):
        best_feature = feature_capture_candidates[0]
        raise ValueError(
            "feature_barcode_capture_library: translated-whitelist chemistry "
            f"{best_feature['chemistry']} scored {float(best_feature['score']):.3f} "
            f">= {min_barcode_match_rate:.3f} while the best GEX chemistry scored "
            f"{best_gex_score:.3f}; Capture Sequence feature-barcode reads are not GEX "
            "and are excluded from GEX mapping"
        )
    if not candidates:
    ...
    return selected["roles"], {
        ...
        "feature_capture_candidates": [
            ...
            for candidate in feature_capture_candidates
        ],
