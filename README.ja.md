...
| `--sdl-stage-timeout-hours` | 1 projectあたりの最終fallbackであるNCBI SDL source file downloadのwall-clock上限（時間）です。上限に達すると完了済み・途中までのfileを保持し、次回の実行で途中のfileから再開します。defaultは `48` です。 |
...
NCBI SDL fallbackには1回の実行あたり48時間のwall-clock上限があります（SDL lookupは1回90秒）。上限に達するとworkerを停止し、完了済み・途中までのfileを保持して、次回の実行では途中のfileから再開します。上限を変えるには `--sdl-stage-timeout-hours` を指定するか、config fileの `[download]` に `sdl_stage_timeout_hours` を書いてください。例:

```bash
uniscflow --mode download ... --sdl-stage-timeout-hours 72
```

どちらも指定しない場合は、`UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS`（秒）が定義されていればその値を使います。lookup上限は `UNISCFLOW_SDL_LOOKUP_TIMEOUT_SECONDS` で調整できます。download stage全体は `docs/workflow.md` を参照してください。

GEO SOFT metadataはGEO query endpointから取得しますが、endpointには短時間の障害があるため、family SOFT archiveへfallbackする前に約55分にわたって再試行します（1秒、2秒、5分、5分、10分、15分、20分）。`UNISCFLOW_GEO_QUERY_RETRY_DELAYS` に秒数をcomma区切りで指定するとこのscheduleを上書きできます（空文字で再試行なし）。例: `export UNISCFLOW_GEO_QUERY_RETRY_DELAYS=1,2,300`
