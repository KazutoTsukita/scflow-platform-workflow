...
Not every input traverses every stage. FASTQ routes use canonical links; eligible BAM inputs with validated raw barcode/UMI tags are supplied to STARsolo as SAM streams without FASTQ conversion. When one sample's selected runs are split between raw FASTQs and an eligible tagged BAM, the FASTQ reads are converted to unaligned SAM records carrying the same raw barcode/UMI tags and both classes are streamed into a single STARsolo run, so UMI deduplication spans every run of the sample. Recognized stops may occur before mapper preparation and produce reports, not matrices. The other modes expose individual stages for inspection or resumption; start with `--mode all` for routine use.
...
| `--parallel` | Parallelism used by download, conversion, and compression steps, and by the post-download FASTQ integrity validation (files are validated concurrently; verdicts are identical to the serial check). |
...
| `--sdl-stage-timeout-hours` | Wall-clock limit for the final NCBI SDL source-file download of one project. When it is reached, completed and partial files are kept and the next run resumes the partial files. Default: `48`. |
...
The NCBI SDL fallback has a wall-clock limit of 48 hours per attempt (each SDL lookup is limited to 90 seconds). When the limit is reached, the workers are stopped, completed and partial files are kept, and the next run resumes the partial files instead of starting over. To change the limit, pass `--sdl-stage-timeout-hours` or set `sdl_stage_timeout_hours` under `[download]` in the config file, for example:

```bash
uniscflow --mode download ... --sdl-stage-timeout-hours 72
```

When neither is set, `UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS` (in seconds) is used if it is defined. `UNISCFLOW_SDL_LOOKUP_TIMEOUT_SECONDS` adjusts the per-lookup limit. See `docs/workflow.md` for the surrounding download stage.

GEO SOFT metadata is fetched from the GEO query endpoint with retries spread over roughly 55 minutes (1 s, 2 s, 5 min, 5 min, 10 min, 15 min, 20 min) before the family SOFT archive is tried, because the endpoint has short outages. `UNISCFLOW_GEO_QUERY_RETRY_DELAYS` overrides the schedule with a comma-separated list of seconds (an empty value disables retries), for example `export UNISCFLOW_GEO_QUERY_RETRY_DELAYS=1,2,300`.
...
UniScFlow checks for submitted BAMs by default. For available BAM rows, it downloads and validates the files and inspects their SAM tags. Eligible BAMs require raw barcode/UMI sequence and quality tags. Runs not covered by usable current inputs proceed through SRA retrieval and missing-run fallbacks. If a sample ends up with both eligible tagged BAM runs and raw FASTQ runs, the mapper does not choose one class or map them separately (which would count each UMI once per class): the FASTQ reads are converted on the fly into unaligned SAM records with `CR/UR` and the BAM's quality tag pair, the BAM's raw barcode/UMI lengths are checked against the FASTQ chemistry, and one STARsolo command consumes the combined stream. The command records the stream status and fails if the conversion or BAM read-out ended early. Use `--no-resolve-bam` to disable the initial BAM-detection step.
