...
def apply_overrides(config: dict, args: argparse.Namespace) -> None:
    ...
    if args.sdl_stage_timeout_hours is not None:
        download["sdl_stage_timeout_hours"] = args.sdl_stage_timeout_hours
...
def download_command(config: dict, project_id: str) -> list[str]:
    ...
    if download.get("sdl_stage_timeout_hours") is not None:
        command.append(f"sdl_stage_timeout_seconds={max(1, round(download['sdl_stage_timeout_hours'] * 3600))}")
...
def validate_runtime_config(config: dict) -> None:
    ...
    if config.get("download", {}).get("sdl_stage_timeout_hours") is not None:
        positive["download.sdl_stage_timeout_hours"] = config["download"]["sdl_stage_timeout_hours"]
...
def _main() -> int:
    ...
    parser.add_argument("--sdl-stage-timeout-hours", type=float, help="Wall-clock limit, in hours, for the final NCBI SDL source-file download of one project. When it is reached, completed and partial files are kept and the next run resumes them. Default: 48 (UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS, in seconds, applies when this option and the config value are unset).")
    ...
    if args.sdl_stage_timeout_hours is not None and not (math.isfinite(args.sdl_stage_timeout_hours) and args.sdl_stage_timeout_hours > 0):
        parser.error("--sdl-stage-timeout-hours must be a finite number > 0")
