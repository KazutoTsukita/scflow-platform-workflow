...
class RuntimeTests(unittest.TestCase):
    ...
    def test_sdl_stage_timeout_hours_is_forwarded_to_shell_in_seconds(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = base_config(Path(temporary))
            unset = uniscflow.download_command(config, "1")
            config["download"]["sdl_stage_timeout_hours"] = 72
            longer = uniscflow.download_command(config, "1")
            config["download"]["sdl_stage_timeout_hours"] = 0.5
            shorter = uniscflow.download_command(config, "1")
        self.assertFalse(any(str(value).startswith("sdl_stage_timeout_seconds=") for value in unset))
        self.assertIn("sdl_stage_timeout_seconds=259200", longer)
        self.assertIn("sdl_stage_timeout_seconds=1800", shorter)

    def test_sdl_stage_timeout_hours_must_be_a_positive_finite_number(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = base_config(Path(temporary))
            uniscflow.validate_runtime_config(config)
            for value in (0, -1, float("nan"), float("inf"), True, "48"):
                config["download"]["sdl_stage_timeout_hours"] = value
                with self.subTest(value=value), self.assertRaises(ValueError):
                    uniscflow.validate_runtime_config(config)
            config["download"]["sdl_stage_timeout_hours"] = 48
            uniscflow.validate_runtime_config(config)

    def test_sdl_stage_timeout_hours_option_reaches_the_download_command(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = base_config(Path(temporary))
            config["project"]["ids"] = ["1"]
            stdout = io.StringIO()
            argv = ["uniscflow", "--mode", "plan", "--ids", "1", "--no-logo", "--sdl-stage-timeout-hours", "72"]
            with (
                mock.patch.object(sys, "argv", argv),
                mock.patch.object(uniscflow, "load_config", return_value=copy.deepcopy(config)),
                mock.patch.object(uniscflow, "ensure_dirs"),
                ...
                uniscflow.main()
            self.assertIn("sdl_stage_timeout_seconds=259200", stdout.getvalue())
            for value in ("0", "-2", "nan", "inf"):
                stderr = io.StringIO()
                argv = ["uniscflow", "--mode", "plan", "--ids", "1", "--no-logo", "--sdl-stage-timeout-hours", value]
                with (
                ...
                self.assertIn("--sdl-stage-timeout-hours must be a finite number > 0", stderr.getvalue())
