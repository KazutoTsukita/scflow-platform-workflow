...
class InputIntegrityTests(unittest.TestCase):
    ...
    def test_mapper_preparation_maps_bam_fastq_split_within_one_gsm_in_one_starsolo_run(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            project = root / "raw" / "prjna1"
            ...
            self.assertEqual(rows[0]["status"], "script_generated", rows[0]["reason"])
            self.assertEqual(set(rows[0]["run_accessions"].split(",")), {"SRR1", "SRR2"})
            command = (Path(rows[0]["mapper_input_dir"]) / "command.sh").read_text()
            # Neither input class is dropped: FASTQ reads become tagged SAM records and are
            # streamed together with the BAM records into one STARsolo run (one UMI dedup).
            self.assertEqual(command.count("--runThreadN"), 1)
            self.assertIn("fastq_pair_to_tagged_sam", command)
            self.assertIn("fastqs/GSM_MIXED_S1_L001_R1_001.fastq.gz", command)
            self.assertIn(f"view -F 0x900 {bam}", command)
            self.assertIn("--readFilesType SAM SE", command)
            self.assertIn("--soloInputSAMattrBarcodeSeq CR UR", command)
            profile = json.loads((Path(rows[0]["mapper_input_dir"]) / "platform_profile.json").read_text())
            self.assertEqual(profile["mixed_raw_inputs"]["mode"], "single_starsolo_run_over_tagged_sam_stream")
            self.assertTrue(any(w.startswith("mixed_fastq_bam_single_stream") for w in profile["input_warnings"]))
