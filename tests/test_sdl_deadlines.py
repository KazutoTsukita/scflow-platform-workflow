...
'''
        for missing, coverage, expected, limit in (('', '0', 0, ''), ('SRR2', '0', 0, ''), ('SRR2', '1', 7, ''),
                                                   ('', '1', 7, ''), ('SRR2', '0', 0, '7200')):
            with self.subTest(missing=missing, coverage=coverage, limit=limit):
                result = subprocess.run([shutil.which('bash') or 'bash', '-c', harness + block],
                                        env=dict(os.environ, MISSING=missing, COVERAGE_RC=coverage,
                                                 sdl_stage_timeout_seconds=limit),
                ...
                if missing:
                    ...
                    self.assertEqual('--stage-timeout-seconds 7200' in result.stdout, bool(limit))
                    self.assertEqual('--stage-timeout-seconds' in result.stdout, bool(limit))

    def test_wrapper_rejects_a_malformed_stage_limit_before_any_work(self):
        path = Path(self.sdl.__file__).with_name('All_in_one_download_NCBI.sh')
        for value, rejected in (('6h', True), ('0', True), ('1.5', True), ('-1', True), ('7200', False)):
            with self.subTest(value=value):
                result = subprocess.run([shutil.which('bash') or 'bash', str(path), 'id=1',
                                         f'sdl_stage_timeout_seconds={value}'],
                                        text=True, capture_output=True, timeout=10)
                self.assertEqual(result.returncode, 1, result.stderr)
                self.assertEqual('sdl_stage_timeout_seconds must be a positive integer' in result.stdout, rejected)
                self.assertEqual('Missing required arguments' in result.stdout, not rejected)

    def test_stage_limit_defaults_to_48_hours_and_the_environment_still_applies(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            table = root / 'runs.tsv'
            table.write_text('run_accession\tsample_alias\nSRR1\tGSM1\n')
            environ = {k: v for k, v in os.environ.items() if k != 'UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS'}
            for env, flags, expected in (({}, [], 48 * 3600),
                                         ({'UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS': '3600'}, [], 3600),
                                         ({'UNISCFLOW_SDL_STAGE_TIMEOUT_SECONDS': '3600'},
                                          ['--stage-timeout-seconds', '7200'], 7200)):
                seen = []
                def collect(rows, args):
                    seen.append(args.stage_timeout_seconds)
                    return [dict(run_accession='SRR1', sample='GSM1', status='failed', reason='sdl_lookup_failed')]
                argv = ['sdl', '--filereport', str(table), '--output-dir', str(root / 'raw')] + flags
                with self.subTest(env=env, flags=flags), patch.dict(os.environ, dict(environ, **env), clear=True), \
                        patch.object(sys, 'argv', argv), patch.object(self.sdl, 'collect_results', side_effect=collect), \
                        contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
                    self.sdl.main()
                    self.assertEqual(seen, [expected])

    def test_stage_timeout_tells_the_user_how_to_raise_the_limit(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            table = root / 'runs.tsv'
            table.write_text('run_accession\tsample_alias\nSRR1\tGSM1\nSRR2\tGSM2\n')
            results = [dict(run_accession='SRR1', sample='GSM1', status='failed', reason='sdl_stage_timeout'),
                       dict(run_accession='SRR2', sample='GSM2', status='failed', reason='sdl_lookup_failed')]
            stderr = io.StringIO()
            argv = ['sdl', '--filereport', str(table), '--output-dir', str(root / 'raw'), '--stage-timeout-seconds', '21600']
            with patch.object(sys, 'argv', argv), patch.object(self.sdl, 'collect_results', return_value=results), \
                    contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(stderr):
                self.assertEqual(self.sdl.main(), 1)
            self.assertIn('reached its 6 h limit with 1 run(s) unfinished', stderr.getvalue())
            self.assertIn('--sdl-stage-timeout-hours', stderr.getvalue())
